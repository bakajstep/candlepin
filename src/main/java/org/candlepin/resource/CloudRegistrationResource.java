...
public class CloudRegistrationResource implements CloudRegistrationApi {
    ...
    private final CloudAuthTokenUtil tokenUtil;
    ...
    public CloudRegistrationResource(Configuration config, I18n i18n,
        ...
        PoolCurator poolCurator, JobManager jobManager, CloudAuthTokenUtil tokenUtil) {
    ...
    public Response cloudAuthorize(CloudRegistrationDTO cloudRegistrationDTO, Integer version) {
        ...
        try {
            ...
            if (version == null || version == 1) {
                ...
                String token = tokenUtil.buildStandardRegistrationToken(principal, ownerKey);
    ...
    private CloudAuthenticationResultDTO processAdapterAuthResult(Principal principal,
        ...
        String anonymousConsumerUuid = existingAnonConsumer == null ? null : existingAnonConsumer.getUuid();
        boolean ownerAndPoolExists = poolCurator.hasPoolForProduct(ownerKey, authResult.getProductId());
        if ((!ownerAndPoolExists && existingAnonConsumer == null) || createAnonConsumer) {
        ...
        String token = anonymousConsumerUuid == null ?
            this.tokenUtil.buildStandardRegistrationToken(principal, ownerKey) :
            this.tokenUtil.buildAnonymousRegistrationToken(principal, anonymousConsumerUuid);

        CloudAuthTokenType tokenType = anonymousConsumerUuid == null ?
            CloudAuthTokenType.STANDARD :
            CloudAuthTokenType.ANONYMOUS;

        return new CloudAuthenticationResultDTO()
            .ownerKey(ownerKey)
